...
const ProfileCard = ({ toggleCard, isCardVisible, userData, commonGroups }) => {
  const {
    username,
    email,
    profilePicturePath,
  ...
  return (
    <div
      className="overlay fixed inset-0 z-50 transition-opacity duration-300 -ml-3 -mt-8"
      onClick={toggleCard}
    >
      <div
        className={`flex ml-110 border-1 border-gray-700 bg-gray-800 flex-col gap-4 p-6 rounded-md shadow-lg max-w-lg overflow-y-auto ${
          isCardVisible
            && "opacity-100 translate-y-12 slide-down"
        }`}
      ...
      >
        ...
        <div className="rounded-lg flex items-center">
          ...
          <div className="flex flex-col gap-3 text-left ml-4">
            ...
            <div className="flex items-center text-sm text-gray-400">
              <FaLocationDot />
              <p>{country || "Laughtale"}</p>
            </div>
          </div>

          {/* Badges Section */}
          <div className="rounded-lg ml-6 w-50 overflow-x-auto hide-scrollbar">
            <h2 className="text-xl text-center font-semibold text-pink-500 mb-2">Badges</h2>
        ...
        <div className="rounded-lg p-6">
          ...
          <p className="text-base text-justify border-t-2 rounded-md border-cyan-500 py-3">
            ...
            . I'm actually an{" "}
            <span className="text-cyan-400">{personality}</span>. Feel free to
