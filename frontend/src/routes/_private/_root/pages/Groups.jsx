...
const Groups = () => {
  ...
  const toggleSearchBar = () => {
    ...
    // reset the search query and match indices
    setSearchQuery("");
  ...
  const handleCreateGroup = async (formData) => {
    try {
      const response = await axiosInstance.post("/groups/new", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      toggleForm();
      const createdGroup = response.data;
      subscribeToGroup(socket, createdGroup);
  ...
  const handleAskToJoinGroup = async (groupId) => {
    try {
      const response = await axiosInstance.patch(`/groups/join/${groupId}`);
      setCurrentLearnersRequests((prev) => [...prev, response.data]);
      toggleForm();
    } catch (error) {
  ...
  const handleCancelJoinGroup = async (requestId) => {
    try {
      await axiosInstance.patch(`/groups/cancel/${requestId}`);
  ...
  const handleJoinGroupRequest = async (action, requestId) => {
    try {
      await axiosInstance.patch(`/groups/${action}/${requestId}`);
  ...
  const handleShowProfile = async (username) => {
    ...
    try {
      const profileData = await axiosInstance.get(
        `/learners/profile/${username}`
      );
      const commonGroupsData = await axiosInstance.get(
        `/groups/common/${username}`
      );
      if (profileData) setAnotherProfile(profileData.data);
      if (commonGroupsData) setCommonGroups(commonGroupsData.data);
      toggleProfileCard();
    } catch (error) {
  ...
  return (
    <div className="flex flex-1 w-full min-h-screen">
      ...
      <div className="flex flex-col gap-4 border-r-1 border-black w-2/7 min-h-screen p-4">
        ...
        {(othersRequests.length > 0 || currentLearnersRequests.length > 0) && (
          <div className="flex flex-col gap-3">
                  ...
                  >
                    <div>
                      <button
                        ...
                        onClick={() =>
                          handleShowProfile(r.requestLearnerUsername)
                        }
                      >
                        {r.requestLearnerUsername}
                      </button>
            ...
            {currentLearnersRequests.length > 0 && (
              <div className="flex flex-col">
                <h5 className="text-md mb-1">Your requests</h5>
                {currentLearnersRequests.map((r, index) => (
                  <div
                    key={index}
              ...
              return (
                ...
                >
                  <MessageBubble
                    ...
                    isFirstMessage={
                      index === 0 ||
                      message.senderUsername !== arr[index - 1]?.senderUsername
                    }
