...
const EditProfile = () => {
  ...
  const { user, isFetchingUserData, fetchUser } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    const validations = [
      [!formData.country, "Please select a country."],
      [!formData.personality, "Please select a personality type."],
      [
    ...
    try {
      ...
      await axiosInstance.put("/learners/update-learning-preference", {
  ...
  const formattedProfilePath = formData.profilePictureUrl
    ? formatImagePath(formData.profilePictureUrl)
    : null;

  // wait for user data to load
  if (isFetchingUserData || !user || !user.learningPreference) {
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="flex items-center">
                  <Uploader
                    initialImage={formattedProfilePath}
                    ...
                    <div className="grid grid-cols-2 gap-2 max-h-40 overflow-y-auto hide-scrollbar pr-1">
                      {introvertTypes.map((type) => (
                        <PersonalityTypeCard
                          key={type.value}
                          type={type}
                          isSelected={formData.personality === type.value}
                          onSelect={(value) =>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-96 overflow-y-auto hide-scrollbar pr-2">
                {learningStyles.map((style) => (
                  <LearningStyleCard
                    key={style.value}
                    style={style}
                    isSelected={formData.learningStyles.includes(style.value)}
                    onToggle={() =>
                      handleUpdateField(
                        "learningStyles",
                        formData.learningStyles.includes(style.value)
                          ? formData.learningStyles.filter(
                              (s) => s !== style.value
                            )
                          : [...formData.learningStyles, style.value]
                      )
                    }
                  />
                ))}
